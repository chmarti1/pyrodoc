...
Once a thermodynamic model permits changes of phase, it makes little sense to specify the state in terms of temperature and pressure.  After all, boiling represents a massive isobaric and isothermal discontinuity in all other properties.  On the other hand, using density requires no such discontinuity, and it even permits modeling meta-stable states.  For this reason, contemporary multi-phase models use temperature and density to specify the thermodynamic state.

Historically, there have been countless approaches to the problem, beginning with the so-called ``viral'' terms inspired by Van der Waals forces in early work on the kinetic theory of gases.  This approach began an evolution of increasingly complex variants of the ideal gas law, from which pressure can be calculated form increasingly complicated formula in terms of temperature and density.  In this approach, parallel models for the specific heat are needed to establish a total substance model.  The so-called ``Span and Wagner'' models in broad use today manage to address all of these issues in a single, albeit rather cumbersome formula.
...
The models supported by \PM's multi-phase classes uses a single formula for Helmholtz free energy as a function of temperature and density,
\begin{align}
f(T,\rho) &\equiv e(T,\rho) - T s(T,\rho)\\
 &= h - \frac{p}{\rho} - T s.
\end{align}

The impressive body of data on which \PM\ draws has been accumulated as follows.  First, individual researchers accumulate bodies of raw experimental data describing the properties like specific heat, pressure, phase-change boundary, critical point, etc. with as as great accuracy as is feasible.  The team wishing to produce a free energy model must accumulate these data, compare them for consistency, and make judgments about which are to be trusted, which discarded, and which kept with some skepticism.  A large bank of terms for an expansion representing $f(T,\rho)$ (described below) are used to fit raw data in a regression model, and the number of terms is iteratively reduced until the precision of the model drops to roughly match the uncertainty of the original data.  In well-studied cases like cryogenic nitrogen, this may be on the order of 0.01\%.  In other cases, like some more esoteric refrigerants, this might be on the order of 1\% or 2\%.

If the uncertainty implied in the model's calculation is of concern to the reader, it will be important to obtain the original publication to thoroughly understand the model's strengths and weaknesses.  This topic is far too deep, and \PM\ includes far too many individual substance models to go into this kind of detail on each one.

\subsection{Nondimensionalization}\label{sec:mp:nd}

These models are built from a nondimensional formulation for free energy.  This means the terms can be scaled to be more or less on the order of unity, and the choice of molar versus mass density is irrelevant.  Many authors use $\alpha$ to represent a nondimensional free energy, and we adopt the same here.
\begin{subequations}
\begin{align}
...
\begin{align}
\alpha(\tau, \delta) = \alpha^o(\tau, \delta) + \alpha^r(\tau, \delta)\label{eqn:mp:atotal}
\end{align}
...
The second group of terms, $\alpha^r_1$, is a Gaussian function multiplied by powers of $\tau$ and $\delta$.  It was first introduced in 1991 by Setzmann et al. to better model the severely distorted surface that occurs near the critical point in oxygen.  Most models only use a few of these terms, and they are usually centered (by $\epsilon$ and $\gamma$) near the critical point.  From the 1991 paper, ``The opinion has been widely held that analytic equations of state covering the whole fluid region are not able to represent the properties in the critical region.'' The authors continue to say, ``We will show that our new equation is able to represent the existing experimental information on the thermodynamic properties of methane in the critical
...
In \PM, the Span and Wagner models are implemented in classes \texttt{mp1} and \texttt{mp2}.  These classes use wildly different approaches to establishing the state from the properties provided by the user, but there are quite a few common elements between them.  See Section \ref{ch:regdat:data} for more information on \PM\ data, and the individual classes are described in the following sections.

\subsection{The model coefficients}

In version 3.0.1, the multi-phase coefficient organization changed significantly to make data entry faster and less error prone.  Terms are organized into ``groups'' based on their characteristics.  At the top level of the multi-phase data dictionaries, there are two entries; the \texttt{"IGgroup"} representing the ideal gas group of terms, and the \texttt{"Rgroup"} representing the residual group terms from Equation \ref{eqn:mp:atotal}.

The \texttt{"IGgroup"} is a dictionary, which further allows entries \texttt{"logt"}, \texttt{"tlogt"}, \texttt{"group0"}, and \texttt{"group1"}, corresponding to the individual portions of Equation \ref{eqn:mp:ao}.  They are all optional, and their respective terms will be presumed zero if they are omitted.  The \texttt{logt} and \texttt{tlogt} entries are scalar coefficients for $\log(\tau)$ and $\tau \log(\tau)$ respectively.  The \texttt{group0} entry is a 1-D polynomial (see Section \ref{sec:num:poly1}) defining $\alpha^\circ_0(\tau)$ from Equation \ref{eqn:mp:ao:0}.  The \texttt{group1} value is expected to be a list of two-element lists or tuples used to define the quantum terms in Equation \ref{eqn:mp:ao:1}.
...
"IGgroup" : {
    "logt" : <c0>,
    "tlogt" : <c1>,
    "group0" : <1D polynomial definition>,
    "group1" : [[<m0>, <b0>], [<m1>, <b1>], <...>]
}
\end{lstlisting}

Within the \texttt{"Rgroup"} dictionary, the \texttt{"group0"} member contains a list of 2D polynomials (see Section \ref{sec:num:poly2}).  Each a polynomial, $p_k(\tau,\delta)$, corresponds to an exponential term, $\exp(-\delta^k) p_k(\tau, \delta)$ in Equation \ref{mp:ar:0}.  They are listed in order, so the first polynomial definition corresponds to $k=0$, the second to $k=1$, and so on.  The \texttt{"group1"} and \texttt{"group2"} members are nested lists with each containing the necessary coefficients and exponents to construct the terms of $\alpha^r_1$ and $\alpha^r_2$ respectively.  
% This comment numbers the code columns.  There are 53 before a line overrun.
%        1         2         3         4         5
%2345678901234567890123456789012345678901234567890123
\begin{lstlisting}[language=Python]
...
Prior to version 3.0.1, the residual and ideal gas groups required \texttt{"Tscale"} and \texttt{"dscale"} entries to define the values used to nondimensionalize the models.  Their use is deprecated as it creates potentially problematic disagreements between the model's critical point and the critical point defined by the top-level data set.  However, if they are specified, they are still honored for now.  Future releases of \PM\ may change this behavior.

\subsection{The \texttt{mp1} class}

The \texttt{mp1} class is the older of the two multi-phase classes in \PM.  Its dictionary definition (in JSON) is in the form below.  To ensure interoperable unit values, the dataset uses J for energy, kg m$^{-3}$ for density, K for temperature, Pa for pressure, and J kg$^{-1}$ K$^{-1}$ for gas constant.
...
\begin{lstlisting}[language=Python]
{
    "id"    : <substance ID string>
    "class" : "mp2"
    "doc"   : <documentation string>
    "names" : [<string>, <string>, ...  # optional
    "inchi" : <InChI string>            # optional
    "cas"   : <CAS ID string>           # optional
    "atoms" : { <dictionary of atoms> } # optional
    "Tlim"  : [Tlow, Thigh]
    "plim"  : [plow, phigh]
    "dlim"  : [dlow, dhigh]
    "mw"    : <molecular weight>
    ...
    "Tt"    : <triple temperature>
    "pt"    : <triple pressure>
    "IGgroup"  : { <ideal gas dictionary> }
    "Rgroup"   : { <residual dictionary>  }
    "PSgroup"  : { <saturation pressure model> }
    "DSLgroup" : { <saturated liquid model> }
    "DSVgroup" : { <saturated vapor model> }
}
\end{lstlisting}

Many original sources for equation-of-state models include polynomial fits for saturation properties that are so accurate, they can even be implemented without applying the Maxwell criteria.  These are encoded in the \texttt{"PSgroup"}, \texttt{"DSLgroup"}, and \texttt{"DSVgroup"} entries as saturation properties as a function of temperature.  The \texttt{mp1} class exposes these properties through methods: \texttt{ds}, \texttt{ps}, \texttt{Ts}, \texttt{es}, \texttt{hs}, and \texttt{ss}.  
...
These saturation models are evaluated by inner methods, \verb|_dsV()|, \verb|_dsL()|, and \verb|_ps()|, which accept a temperature array as their only argument.

Saturation and ordinary property calculations are performed by inner methods, like \verb|_e()|, \verb|_cp()|, or \verb|_s()|, accept temperature and density arrays, and calculate their respective states in the appropriate default units.  The equivalent user-layer methods with no underscore, like \texttt{e()}, \texttt{cp()}, or \texttt{s()}, pass the user's raw arguments to \verb|_argparse()|, which is responsible for determining the thermodynamic state the user is specifying.  

If more information is needed, the in-line help for each of the methods gives a full description of its purpose and arguments.

\subsection{The \texttt{mp2} class}

The \texttt{mp2} class is the newer of the two multi-phase classes, first released in version 3.0.1.  It design was motivated by two goals.  Firstly, the \texttt{mp1} class dependence on ancillary formulae to determine saturation states meant it was limited to sources that include those models.  Second, there is a significant amount of redundant (and potentially contradictory) information designed into the \texttt{mp1} approach, which creates potential issues.  For example, the equation of state, in theory, completely determines the saturation conditions, making the redundant saturation polynomials in (hopefully small) contradiction with the EOS model.

Its dictionary definition (in JSON) is in the form below.  To ensure interoperable unit values, the dataset uses J for energy, kg m$^{-3}$ for density, K for temperature, Pa for pressure, and J kg$^{-1}$ K$^{-1}$ for gas constant.
...
\begin{lstlisting}[language=Python]
{
    ...
    "dlim"  : [dlow, dhigh]
    "mw"    : <molecular weight>
    "R"     : <Gas Constant Scalar>     # optional
    "Tc"    : <critical temperature>
    "dc"    : <critical density>
    "Tt"    : <triple temperature>      # optional
    "IGgroup" : { <ideal gas dictionary> }
    "Rgroup"  : { <residual dictionary>  }
}
\end{lstlisting}

Most of the optional entries are for data features that will simply be disabled if they are not provided.  For example, the \texttt{triple()} method raises an exception if the triple point temperature is not provided.  The name, CAS, and InChI strings all provide data to their respective \texttt{names()}, \texttt{cas()}, and \texttt{inchi()} methods, and are not required for class instances to function properly.  

When omitted, the ideal gas constant is calculated precisely from the \verb|pm.units.const_Ru| value and the molecular mass, but allowing data authors to specify it explicitly is a vital feature for empowering the model to match precisely with the original publication.  Many authors have optimized their models with rounded values for the gas constant, so to encode a model that agrees precisely with their model design, it is also important to use a truncated gas constant value.

Note that all pressure data (critical and triple) are absent, since they are determined from the equation of state model.  The chief challenge posed by a theoretically minimal data set is how to implement it in a way that is not horribly slow or numerically unstable.

At retrieval using \texttt{get()} or the first time its \verb|_argparse()| method is called, a \texttt{mp2} instance's \verb|mp2._build()| method is called to construct a set of saturation and property look-up tables.  The process takes some computation time, so it is not performed on all substances -- only those retrieved for use.  If necessary, the \verb|_build()| method calls (in strict order) \verb|_build_sattab()| then \verb|_build_tab()|, which create instance attributes \verb|_sattable| and \verb|_table| respectively, which are dictionaries of property arrays.  

The various property functions (especially \verb|_argparse()|) makes heavy use of these tables to establish initial guesses very close to an acceptable answer.  This allows the various iterative algorithms to abandon the \verb|_hybrid1()| numerical tool, developed for global numerical stability in \texttt{mp1}, in favor of basic Newton-Rhapson iteration.

\textbf{Constructing the saturation table} is the first and slowest part of table construction.  When it is complete, it contains entries, which are identically sized 1D Numpy arrays representing the columns of a traditional saturation table.
% This comment numbers the code columns.  There are 53 before a line overrun.
%        1         2         3         4         5
%2345678901234567890123456789012345678901234567890123
\begin{lstlisting}[language=Python]
<substance>._sattable = {
    ...
    'g'  : <saturation gibbs energy>
    'dL' : <saturated liquid density>, 
    'dV' : <saturated vapor density>,
    'eL' : <saturated liquid int. energy>,
    'eV' : <saturated vapor int. energy>, 
    'hL' : <saturated liquid enthalpy>, 
    'hV' : <saturated vapor enthalpy>,
    'sL' : <saturated liquid entropy>, 
    'sV' : <saturated vapor entropy>, 
    'fL' : <saturated liquid free energy>, 
    'fV' : <saturated vapor free energy>
}
\end{lstlisting}

After initialization, the \texttt{mp2} instance has no explicit information on its saturation properties -- it must deduce the table from the critical temperature and density and from the Maxwell criteria in Equations \ref{eqn:mp:maxwellg} and \ref{eqn:mp:maxwellp}.  Especially near the critical point, guesses need to be extremely near to their actual values, lest they diverge.  To ensure numerical stability, \verb|_build_sattab()| calculates the table one point at a time, generating a guess by linear extrapolation from the last converged point.

Extrapolation to the next point is accomplished from derivatives of the Maxwell criteria.  These represent two constraints with three variables, $T$, $\rho_L$, and $\rho_V$, so the tangent to the curve they form in 3-space can be calculated from their derivatives.  For ease of notation,
\begin{subequations}
\begin{align}
...
\textbf{Constructing the general property table} requires a rectangular grid of temperature and density values.  After \verb|_build_tab()| is complete, the \verb|_table| attribute of the substance instance appears as below.
% This comment numbers the code columns.  There are 53 before a line overrun.
%        1         2         3         4         5
%2345678901234567890123456789012345678901234567890123
\begin{lstlisting}[language=Python]
<substance>._table = {
    'T'  : <m-element 1D array>,
    'd'  : <n-element 1D array>,
    'cI' : (<Tci>, <dci>),
    'p'  : <m x n 2D array>,
    'e'  : <m x n 2D array>,
    'h'  : <m x n 2D array>,
...
The grid is constructed automatically from the saturation curve using a series of rules:
\begin{enumerate}
\item Density and temperature arrays must be in ascending order.
\item No step between any two adjacent $T$ or $\rho$ values may be larger than 1\% of the model's valid range.
\item The temperature and density arrays must span between their respective minima and maxima.
\item For each temperature below the critical temperature, the density array must contain values corresponding precisely to the liquid and vapor saturation densities at that temperature.
...
\item The critical temperature and density must be represented precisely in their respective arrays.
\end{enumerate}
...
The location of the critical point in the tables is recorded in the \texttt{"cI"} entry, as a tuple with the temperature index first and the density index second.
% This comment numbers the code columns.  There are 53 before a line overrun.
%        1         2         3         4         5
%2345678901234567890123456789012345678901234567890123
\begin{lstlisting}[language=Python]
...
>>> pc = _table['p'][ii,jj]
\end{lstlisting}
Thanks to rules 4 and 5 above, it is always trivial to identify grid points precisely on the saturation line.  If we were to move $k$ indices below the critical temperature, the saturation properties are
% This comment numbers the code columns.  There are 53 before a line overrun.
%        1         2         3         4         5
%2345678901234567890123456789012345678901234567890123
\begin{lstlisting}[language=Python]
...
>>> dsL = _table['d'][jj+k]
>>> dsV = _table['d'][jj-k]
>>> hsL = _table['h'][ii-k, jj+k]
>>> hsV = _table['h'][ii-k, jj-k]
\end{lstlisting}

This approach naturally produces a high density of grid lines in regions where the saturation curves cause properties to be very steep.  Figure \ref{fig:mp:eTd} shows the internal energy table automatically generated for acetone.  There is a tight grouping of temperature grid lines near the critical point, but there is also a remarkably sharp vertical slope under ``the dome'' near the vapor line caused by the severely dissimilar densities between liquids and vapors far below the critical point.  It only takes a small amount of vapor to significantly drop the density, while adding relatively little to the mixture's energy.  It is not until the density has diminished to a tiny fraction (often around 0.2\% or so) of its liquid value that the higher vapor energy dominates.  Note that this grid generation strategy naturally densely populates this region with grid lines, producing a well formed, though nearly vertical surface.
...
\textbf{Table lookup primitives} are responsible for casing out the various ways to interrogate the tables.  Numpy and Scipy offer interpolation algorithms, but they require the source data to be sorted.  This is fine when working from temperature and density, but they do not permit ``reverse'' table lookups.  The ``mapsearch'' algorithms address this issue.  Since their arrays are one-dimensional, the saturation tables use \verb|_mapsearch1()|.  The general property tables, on the other hand, use \verb|_mapsearch2()|, \verb|_Tmapsearch2()|, and \verb|_dmapsearch2()|.  Their inline documentation offers a detailed description of how their algorithms work.

\textbf{Iterative routines} in \texttt{mp2} are not in specialized methods like \verb|_hybrid1| described in Chapter \ref{ch:num}.  Becuse Newton-Rhapson algorithms are so simple to code, a myriad of specialized inner routines are responsible for performing iteration custom to the particular job.  These are organized by which property is held constant while others are allowed to change.  The saturation algorithms include \verb|_Tsatiter()|, \verb|_dVsatiter()|, \verb|_dLsatiter()|, \verb|_dsatiter()| (which preserves mixture density),  \verb|_psatiter()|, \verb|_gsatiter()|, and \verb|_satiter2()| (which preserves two arbitrary mixture properties).  The generalized property iteration algorithms require only \verb|_Titer()| (holding temperature constant), \verb|_diter()| (holding density constant), and \verb|_iter2()| (simultaneous iteration on both temperature and density).

\textbf{Inner property methods} in \texttt{mp2} behave fundamentally differently from their equivalents in \texttt{mp1}.  Rather than accepting a state definition in terms of temperature and density, the inner methods like \verb|_e()|, \verb|_cp()|, \verb|_h()|, or \verb|_s()| accept the explicit values of $\alpha(\tau, \delta)$ and its derivatives, as returned by a new inner method, \verb|_ff()|.  Though this makes the call signatures more cumbersome, it saves countless back-end methods from redundantly performing the most computationally expensive part of the process -- evaluation of $\alpha$.  Instead, methods that require multiple simultaneous properties (like while iterating on saturation states) merely make a single call to \verb|_ff()| and share its output with the relevant inner property methods.  All of this nuance is opaque to the user -- the user layer property methods function just like the other \PM\ classes.

\subsection{Benchmarking}

Improved computational speed was always a goal in the development of \texttt{mp2}, but it was secondary to broadening the number of substances included in \PM.  To gauge the relative strengths and weaknesses of the two classes, benchmarking was performed by passing randomly generated property arrays in different combinations.  The process of calculating properties is divided into two steps: (1) determination of the thermodynamic state in terms of temperature and density ($t_1$), and (2) calculation of the requested properties at that state ($t_2$).  

The relative computational costs of these steps were measured as the average of ten repeated calls to \verb|_argparse()|, followed by ten repeated calls to \verb|state()| with the same datasets.  The time to calculate properties was assessed as the time required for \verb|state()| minus the time required for \verb|_argparse()|.  Tests were performed on a single core of an AMD Ryzen 9 7900 with array sizes ranging from 10 to 100,000.  The results for H$_2$O (representing \texttt{mp1}) and N$_2$ (representing \texttt{mp2}) are shown in Table \ref{tab:mp:benchmark}

\begin{table}
\centering
\caption{Benchmarking tests comparing \texttt{mp1} against \texttt{mp2} for determining the state, $t_1$, and evaluating the state properties, $t_2$.  Values are in microseconds.}\label{tab:mp:benchmark}
\begin{tabular}{|ccc|rrrrr|}
\hline
 & & N & 10 & 100 & 1,000 & 10,000 & 100,000\\
\hline
\multirow{4}*{$(T,d)$} & \multirow{2}*{\texttt{mp1}} & $t_1$ & 88 & 91 & 143 & 565 & 4,862\\
                       &                             & $t_2$ & 1,841 & 1,930 & 2,961 & 9,947 & 110,323\\
\cline{4-8}
                       & \multirow{2}*{\texttt{mp2}} & $t_1$ & 11 & 2,824 & 4,524 & 6,536 & 16,979\\
                       &                             & $t_2$ & 741 & 1,530 & 2,152 & 6,258 & 72,040\\
\hline
\multirow{4}*{$(T,p)$} & \multirow{2}*{\texttt{mp1}} & $t_1$ & 14,092 & 15,160 & 22,958 & 80,303 & 843,870\\
                       &                             & $t_2$ & 1,679 & 1,270 & 1,992 & 6,620 & 87,956\\
\cline{4-8}
                       & \multirow{2}*{\texttt{mp2}} & $t_1$ & 2,358 & 4,268 & 17,889 & 115,795 & 1,16$\times 10^6$\\
                       &                             & $t_2$ & 739 & 765 & 1,295 & 1,138 & 66,845\\
\hline
\multirow{4}*{$(s,p)$} & \multirow{2}*{\texttt{mp1}} & $t_1$ & 190,888 & 270,547 & 399,682 & 1.10$\times 10^6$ & 9.03$\times 10^6$\\
                       &                             & $t_2$ & 1,347 & 1,006 & 3,757 & 19,521 & 223,192\\
\cline{4-8}
                       & \multirow{2}*{\texttt{mp2}} & $t_1$ & 2,229 & 15,008 & 74,794 & 643,537 & 6.42$\times 10^6$\\
                       &                             & $t_2$ & 748 & 1,570 & 571 & 4,775 & 61,988\\
\hline
\end{tabular}
\end{table}

In general, \texttt{mp2} scales more poorly with array size than \texttt{mp1}.  This is expected, since many of the mapsearch algorithms are serialized in for loops.  When $(T,\rho)$ property pairs are specified, no iteration is required, so the improved initial guesses of \texttt{mp2} offers no benefit.  Instead, the extra overhead required to look up two-phase mixture information causes it to be somewhat slower.  

It is seen that for any property combination other than $(T,\rho)$, the time required to determine the thermodynamic state, $t_1$, dominates the computational cost.  When iteration on one property is required (e.g. when $(T,p)$ property pairs are specified), \texttt{mp2} is about five times as fast as \texttt{mp1}, slowing to break even between 1,000 and 10,000 elements.  On huge datasets, \texttt{mp1}'s better vectorization makes it somewhat faster, but not much compared to \texttt{mp2}'s advantage in small datasets.

When iteration on two simultaneous properties is required (as in the case of $(s,p)$) \texttt{mp2} is about 100 times faster for small datasets.  That advantage erodes in large data sets, but \texttt{mp1} never managed to overtake \texttt{mp2} in these tests.  The \texttt{mp1} algorithms suffers severely here because it does not perform true 2D numerical inversion.  For numerical stability, nested \verb|_hybrid1| algorithms provide a horribly inefficient, albeit extremely numerically stable facsimile of 2D inversion.  As a result, \texttt{mp2} is far superior in these cases.

Except for users calling the property methods inside of a higher iterative process, these magnitudes of numerical costs are mostly insignificant.  Most users work on small datasets, and will not be bothered by the difference between a 1 millisecond versus a 10 millisecond calculation.  This comparison is entirely for the benefit of users considering doing something a little more adventurous with these models.
