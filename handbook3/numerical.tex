...
The \texttt{json} data representation of a polynomial on two dimensions uses a dictionary to describe pre- and post-exponents, the coefficients, and their corresponding exponents for multiple polynomial parts.  Prior to version 3.0.1, polynomials were defined using a series of nested lists, but the transition to dictionaries dramatically improved human readability of the data, and made manual entry dramatically faster with fewer typos.
...
poly = {
    # Pre- and post-exponents are optional
    'pre': [<a>,<b>],       
    'post': [<alpha>, <beta>],
    # The mandatory coefficient list
    'coef': [
        [<i0>, <j0>, <c0>],
        [<i1>, <j1>, <c1>],
        <...>
...
Each pair of non-negative integer exponents, \texttt{i0}, \texttt{j0}, specifies the existence of an individual term, and the corresponding \texttt{c0} indicates its floating point coefficient.  The exponents must be sorted in descending order by $i$ and then sorted by descending $j$; so every $i$ will be less than or equal to the one above, and each $j$ will be less than the $j$ values above with the same values of $i$.  This approach is a ``sparse'' polynomial definition, where only data for the terms with non-zero coefficients are provided.  By contrast a ``dense'' definition requires a fully populated matrix of coefficients, where each row and column corresponding to a non-negative integer power of $x$ or $y$.  In the sparsely populated polynomials common to the models used in PYroMat, sparse methods are easily more efficient.

It is often preferable (and sometimes essential) to specify different pre- and post-exponents for portions of a polynomial.  The most common motivation comes from terms with floating point exponents that are not efficiently represented as an integer multiple of the same rational.  For example $\tau^{0.221}$ and $\tau^{0.5}$ might be represented with a pre-exponent of 0.001, resulting in terms with exponents 221 and 500.  Such high exponents are numerically problematic and horribly inefficient.  In these cases, sub-polynomial dictionaries can be arranged inside a list -- each dictionary may specify its own pre- and post-exponents.  
...
It is usually a good idea to minimize the number of sub-polynomials in a data model whenever possible.  It may be prudent to experiment with different realizations of the same polynomial to see which is most computationally efficient while still giving accurate results.  The latter becomes an issue when pre-exponents shrink to be very tiny.  Terms like the one below can impart significant numerical error, so the added time for an extra floating point exponent might be worth it.
\begin{align*}
\left(x^\mathrm{tiny\ number}\right)^\mathrm{HUGE\ INTEGER}
\end{align*}

\subsection{Efficient evaluation of the polynomial}

As we have seen in the previous section, a polynomial can be adjusted so that
\begin{align}
p(x,y) = \sum_{i,j} c_{i,j} x^i y^j
\end{align}
where all values of $i$ and $j$ are non-negative integers.  However, evaluating each term individually requires two expensive calls to a \verb|pow| function and two floating point multiplications.
...
The partial derivatives of the polynomial can be efficiently evaluated along with the polynomial itself. To relax the already cumbersome notation, the functional dependencies $(y)$ and $(x,y)$ will be dropped.  For the purpose of thermodynamic property evaluation, only the first two derivatives are needed.  We will denote the partial derivatives with subscripts so that 
\begin{align*}
\frac{\partial p_{i,j}}{\partial x} = p_{i,j|x} && \frac{\partial^2 p_{i,j}}{\partial x^2} = p_{i,j|xx}.
\end{align*}
...
Just like with 2-dimensional polynomials, 1D polynomials allow sub-polynomials, so multiple pre- and post-exponentials can be used within the same polynomial.
