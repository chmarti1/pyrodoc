...
\PM\ handles a wide variety of units automatically.  The configuration system (see Chapter \ref{ch:config}) allows users to select the units they prefer when interacting with the \PM\ user layer.  Meanwhile the raw data that form the back-end substance models are coded in standard units unique to each class.  The \verb|pyromat.units| module provides the tools that translate between them.  Interested users can use the back-end algorithms for their own purposes to convert between most units users need for their calculating pleasures.  
...
There may seem to be a chauvinism in the author's choice of some units to include (such as the esoteric US customary volumetric units), and some of the sources are somewhat biased in favor of the US National Institute of Standards and Technology (NIST).  The author acknowledges this bias.  It is hoped that the reader will be reassured that first and foremost, units common in international scientific and engineering fields were included.  The inclusion of some (perhaps out-dated) units peculiar to the US is mostly an indulgence in the author's curiosity.

\section{Unit definitions}\label{sec:units:def}

The Bureau International des Poids et Mesures (BIPM) International Committee for Weights and Measures (CIPM) is the entity responsible for the definition of the fundamental units on which the SI system is based, and from which almost all other units trace their definition.  Nearly all other units in wide use are now derived from the SI units in some way, so rigorous treatment of their formal definition is worth some attention.  There are countless web-based unit conversion calculators, but few use sufficient care in the derivation of their conversion factors to be trusted for serious work.
