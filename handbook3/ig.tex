...
The ideal gas law is empirical in its origins, originally formulated as an amalgamation of the independent laws of Gay-Lussac, Charles, and Boyle.  The kinetic theory of gases eventually provided an independent formulation based entirely in Newton's laws.  Given its importance to the study of matter and its deeply intuitive nature, it is surprising that few introductory text on thermodynamics gives the subject any treatment.  It is worth a brief summary here.
...
When a gas composed of a monoatomic molecule like argon is heated (without atomic, chemical, or phase changes) the energy can only be stored in the thermal translational energy described in (\ref{eqn:thermal:k}).  A sample of $N$ molecules of such a gas would have thermal energy $N \frac{3}{2} k T$, so the energy of one mole or one gram is
\begin{align}
\overline{e} - \overline{e}_0 &= N_{a} \frac{3}{2} k T = \frac{3}{2} R_u T\\
e - e_0 &= \frac{N_{a}}{W} k T = \frac{3}{2} R T \label{eqn:pg:e}.
\end{align}
See section \ref{sec:units:molar} for definitions of the molar mass, $W$, and the Avagadro number, $N_{a}$.  Here, $e_0$ is the internal energy due to the atomic, chemical or phase changes we have not yet considered.  Note that the gas constants, $R_u$ and $R$, appear naturally in terms of the Boltzmann constant, $k$.
...
\begin{align}
f(T,p) = h(T) - (R+s(T,p))T,\label{eqn:ig:f}
\end{align}
Gibbs energy,
\begin{align}
g(T,p) = h(T) - T s(T,p),\label{eqn:ig:g}
\end{align}
...
specific heat ratio,
\begin{align}
\gamma(T) = \frac{c_p(T)}{c_p(T) - R},\label{eqn:ig:gam}
\end{align}
and others.
...
\begin{align*}
\overline{s}(T,p) &= \overline{s}_\mathrm{mix} + \sum \chi_i\ \overline{s}^\circ_i(T) - R_u \ln\left( \frac{p}{p^\circ_\mathrm{mix}} \right),\label{eqn:ig:igmixs}
\end{align*}
...
Apart from $\overline{s}_\mathrm{mix}$, Equation \ref{eqn:ig:igmixs} is merely a weighted average of the entropies of the individual constituent gases.  Neither a function of temperature nor pressure, the entropy of mixing, $\overline{s}_\mathrm{mix}$, is a constant for a given mixture of constituents, and it represents the entropy created by dispersing gases into one another.  The same gases, neatly partitioned into partitioned thin-walled compartments, and the systems entropy would be correctly calculated without $\overline{s}_\mathrm{mix}$.  

The smallest possible value for $\overline{s}_\mathrm{mix}$ is zero, which is only achieved when $\chi_i$ values are all zero or unity.  Otherwise, since values of $\chi_i$ are positive and less than one, its log will always be negative, and the contribution to $\overline{s}_\mathrm{mix}$ positive.  

The \verb|igmix._build()| method calculates a number of mixture properties that do not depend on state, including $\overline{s}_\mathrm{mix}$ and $p^\circ_\mathrm{mix}$, which are stashed in \verb|_smix| and \verb|_pref_pa| attributes respectively.  The \verb|_build()| method is also responsible for accumulating temperature limits, representing the narrowest of all the constituent gases, stored in \verb|_Tlim|; the mixture's effective molecular weight, stored in \verb|_mw|; and mass-based and mole-based mixture fraction dictionaries, stored in \verb|_y| and \verb|_x| respectively.
