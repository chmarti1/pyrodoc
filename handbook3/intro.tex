...
\item Inverse properties: In order to determine the state (in primary properties) from these properties, \PM\ either needs to perform numerical iteration, or (in rare exceptions) a precise algebraic inverse is available.
    \begin{itemize}
    \item Ideal gases: $\rho$, $e$, $f$, $g$, $h$, $s$,
...
In \PM, a property combination is disallowed if there are at least two distinct states that can be found providing the same combination.  For example, Figure \ref{fig:Thd} shows a surface plot of density plotted against temperature and enthalpy for two-phase acetone.  Except in the liquid region, the surface is very badly behaved numerically, but the important criterion that causes us to reject it as a valid property combination is that it wraps back on itself.  For many values of $T$ and $h$, and there are two values of $\rho$ that produce the same value of $h$.  Therefore, $h,T$ does not specify a unique state.

\begin{figure}
\centering
\includegraphics[width=0.9\linewidth]{figures/Thd}
\caption{A surface plot of density plotted against temperature and enthalpy for a two-phase mixture of acetone.}\label{fig:Thd}
\end{figure}
