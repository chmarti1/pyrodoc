...
Probably, no reader will need to digest this handbook in its entirety.  The goal is to include all the important information in a single portable document.  Chatper \ref{ch:start} on Getting Started is probably the one that most users will need, since it describes installation and basic use of the software.
...
A detailed equation of state model can make equal use of almost any pair of properties to define the others, and since version 2.2.0, \PM\ is quite flexible about which two are used.  While this kind of convenience is part of the purpose of a high-level software tool, users may notice that some combinations are faster than others, some produce more numerical inaccuracies than others, and some are simply not possible.

To help make sense of the different property combinations, \PM\ organizes properties into groups based on the information that can be gained from them.
\begin{itemize}
\item Basic properties: These are the most physically intuitive properties -- usually obtainable from direct physical measurement.  In general, specifying the thermodynamic state with these is faster and more numerically precise.
    \begin{itemize}
    \item Temperature, $T$,
    \item Pressure, $p$,
    \item Density, $\rho$,
    ...
    \item Quality, $x$.
    \end{itemize}
\item Primary properties: Every model uses two basic properties to uniquely define a thermodynamic state, from which all the others can be calculated.  Specifying the thermodynamic state with these is always the fastest, most precise method.
    \begin{itemize}
    \item Ideal gases: $T, p$,
    \item Multi-phase models: $T, \rho$.
    \end{itemize}
\item Energy properties: These are different ways of quantifying the energy contained in the system.  Specifying pairs of energy properties is usually inadequate for defining the thermodynamic state, so it is almost always disallowed.
    \begin{itemize}
    \item Temperature, $T$,
    \item Internal energy, $e$,
    \item Enthalpy, $h$,
    \item Free (Helmholtz) energy, $f$,
    \item Gibbs energy, $g$.
    \end{itemize}
\item Inverse properties: In order to determine the state (in primary properties) from these properties, \PM\ either needs to perform numerical iteration, or (in rare exceptions) a precise algebraic inverse is available.
    \begin{itemize}
    \item Ideal gases: $d$, $e$, $f$, $g$, $h$, $s$,
    \item Multi-phase models: $p$, $e$, $f$, $g$, $h$, $s$.
    \end{itemize}
\end{itemize}

\textbf{Ideal gas property combinations} supported by the \texttt{ig}, \texttt{ig2}, and \texttt{igmix} classes are listed in Table \ref{tab:props:ig}.  Combinations are marked with ``X'' when they would either be self-contradictory or redundant.  Every combination along the diagonal is obviously redundant, but simultaneously specifying specific volume and density is also redundant.  As we show in Chapter \ref{ch:ig}, for an ideal gas, specifying internal energy, enthalpy and temperature are all theoretically equivalent.

\textbf{The multi-phase property combinations} supported by the \texttt{mp1} and \texttt{mp2} classes are shown in Tables \ref{tab:props:mp1} and \ref{tab:props:mp2} respectively.  Almost all combinations that are theoretically valid are supported.  A ``o'' represents a combination that is not supported, though there may be some cases where it is theoretically possible.  The theory behind these combinations is somewhat more nuanced than in ideal gases.

We regard a property combination as theoretically disallowed if there are at least two distinct states that can be found providing the same combination.  For example, Figure \ref{fig:Thd} shows a surface plot of density plotted against temperature and enthalpy for two-phase ammonia.  Except in the liquid region, the surface is very badly behaved numerically, but the important criterion that causes us to reject it as a valid property combination is that it wraps back on itself.  For given values of $T$ and $h$, and there are two values of $\rho$ that produce the same value of $h$.  Therefore, $h,T$ does not specify a unique state.

\begin{figure}
\centering
\includegraphics[width=0.9\linewidth]{figures/Thd}
\caption{A surface plot of density plotted against temperature and enthalpy for a two-phase mixture of ammonia.}\label{fig:Thd}
\end{figure}

Despite this problem, $T,h$ and $T,e$ are both allowed in \texttt{mp1}, and they can only be expected to give reasonable results in (or near) the liquid region.  On the other hand, $f$ and $g$ are not permitted as arguments, and any combination of the inverse group is disallowed for reasons of numerical stability.  For example it is theoretically valid to specify enthalpy and entropy together, but it requires iteration in two dimensions to extract the $T,\rho$ state.  This presents a significant challenge for global numerical stability.

The \texttt{mp2} class addresses the global numerical stability issue by stashing a table of property values that can be referenced to establish initial estimates for properties prior to local numerical polishing.  It recognizes $f$ and $g$ in its arguments, but it avoids the numerical problems seen in \texttt{mp1} by disallowing any combination of properties from the energy group.
...
\hline
  & \texttt{T} & \texttt{p} & \texttt{d} & \texttt{v} & \texttt{e} & \texttt{h} & \texttt{s}\\
\hline  %  & T   p   d   v   h   e   s
...
\begin{table}
\centering
\caption{Property combinations that are supported by the \texttt{mp1} class in version 3.0.1.  \checkmark = supported, X = not valid, o = not supported.}\label{tab:props:mp1}
\begin{tabular}{|c|ccccc|ccc|}
\hline
  & \texttt{T} & \texttt{p} & \texttt{d} & \texttt{v} & \texttt{x} & \texttt{e} & \texttt{h} & \texttt{s}\\
\hline  %  & T   p   d   v   e   h   s
...
\hline
\texttt{e} & \checkmark & \checkmark & \checkmark & \checkmark & X & X & X & o\\
\texttt{h} & \checkmark & \checkmark & \checkmark & \checkmark & X & X & X & o\\
...
\begin{table}
\centering
\caption{Property combinations that are supported by the multi-phase classes in version 3.0.1.  \checkmark = supported, X = not valid, o = not supported, E = energy group.}\label{tab:props:mp2}
\begin{tabular}{|c|ccccc|cc|}
\hline
  & \texttt{T} & \texttt{p} & \texttt{d} & \texttt{v} & \texttt{x} & E & \texttt{s}\\
\hline  %  & T   p   d   v   E   s
\texttt{T} & X & \checkmark & \checkmark & \checkmark & \checkmark & o & \checkmark\\
\texttt{p} & \checkmark & X & \checkmark & \checkmark & \checkmark & \checkmark & \checkmark\\
\texttt{d} & \checkmark & \checkmark & X & X & X & \checkmark & \checkmark\\
\texttt{v} & \checkmark & \checkmark & X & X & X & \checkmark & \checkmark\\
\texttt{x} & \checkmark & \checkmark & X & X & X & X & X \\
\hline
E          & o & \checkmark & \checkmark & \checkmark & X & o & \checkmark\\
\texttt{s} & \checkmark & \checkmark & \checkmark & \checkmark & X & \checkmark & X\\
\hline
\end{tabular}
\end{table}
