...
Y &= y^b.
\end{align}
Then, if the polynomial were also multiplied by post-terms, $x^\alpha$, and $y^\beta$, the new polynomial formed is
\begin{align}
...
As of version 2.3.0, the \texttt{json} data representation of a polynomial on two dimensions uses dictionaries to define the polynomial elements.  Older versions of the \verb|mp1| class used nested lists, but with the addition of \verb|mp2| class, dictionaries were introduced to make the data files more readable (and more writeable).

The simplest polynomial definition is a dictionary with only a coefficient table:
...
\end{lstlisting}
2D polynomail coefficient tables are nested (2D) lists with each row containing 3 elements; $x$-exponent, $y$-exponent, and coefficient.  The $i$ and $j$ exponents must be non-negative integers, while the coefficient may be a float or integer.  The rows \emph{must} be sorted first by descending $x$-exponent and then by descending $y$-exponent.  For example, the polynomial,
\begin{align}
p(x,y) = 2x^2 + 0.5 xy - 2xy^2 + y^3 - 4.2,
\end{align}
is expressed
\begin{lstlisting}{language=Python}
poly = {
...
\end{lstlisting}
The behavior of the multi-phase polynomial algorithms is undefined if the coefficient order is not correctly sorted.

The addition of pre- and post-exponentials is optional, and is done by adding keywords to the dictionary.  The \verb|pre| keyword accepts a two-element list in the form \verb|[a,b]|, while \verb|post| accepts \verb|[alpha, beta]|.  For example,
\begin{lstlisting}{language=Python}
poly = {
    'pre' = [0.5, 1],
    'post' = [-1, 0]
...
\begin{align}
p(x,y) = \frac{1}{\sqrt{x}} - \frac{y}{x}.
\end{align}
Here, the coefficient table defines a basic polynomial,
\begin{align}
p(X,Y) = X - Y
\end{align}
The pre-exponential gives us, $X = \sqrt{x}$ and $Y = y$.  Then, due to the post values, the entire polynomial will be multiplied by $1/x$.  Note that specifying pre-exponentials of 1 and post-exponentials of 0 is the same as specifying none.

In more complex formulae, it is often necessary to break the polynomial into parts with different pre- and post-exponential values.  In this case, the polynomial can be defined as a list of dictionaries, where each dictionary obeys the rules above.

For example, the nested lists below define a polynomial with multiple parts.
\begin{lstlisting}{language=Python}
...
Polynomials of one dimension benefit from the same approach as two-dimensional polynomials, but the algorithm is far simpler.  All the formulae and algorithms that apply to $q$ in the last section may simply be repurposed for a single dimensional polynomial.  The only differences are (1) the pre- and post-exponent values are scalars instead of two-element lists, and (2) each term only needs two parameters instead of three.

\begin{lstlisting}[language=Python]
coef = [ # Optional opening list begins sub-polynomials
